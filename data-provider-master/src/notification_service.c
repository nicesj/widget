...
static void _handler_package_install(struct tcb *tcb, struct packet *packet, void *data)
{
	...
	if (packet_get(packet, "s", &package_name) == 1) {
		...
		notification_setting_insert_package(package_name);
...
static void _handler_package_uninstall(struct tcb *tcb, struct packet *packet, void *data)
{
	int ret = NOTIFICATION_ERROR_NONE;
	char *package_name = NULL;

	DbgPrint("_handler_package_uninstall");
	if (packet_get(packet, "s", &package_name) == 1) {
		DbgPrint("package_name [%s]\n", package_name);

		notification_setting_delete_package(package_name);
...
static int service_thread_main(struct tcb *tcb, struct packet *packet, void *data)
{
	...
	static struct noti_service service_req_no_ack_table[] = {
		...
		{
			.cmd = "package_uninstall",
			.handler = _handler_package_uninstall,
			.rule = NULL,
			.access = NULL,
			.handler_access_error = NULL,
		},
